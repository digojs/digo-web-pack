...
export class Packer {
    ...
    createFile(path: string, content: string, sourceFile?: digo.File, sourceIndex?: number) {
        ...
        if (sourceFile) {
            ...
            result.log = (data: string | Error | digo.FileLogEntry, args?: { [key: string]: any; }, level?: digo.LogLevel) => {
                if (!(data instanceof digo.LogEntry)) {
                    data = new digo.FileLogEntry(sourceFile, data, args);
                }
                if (data.fileName && digo.pathEquals(data.fileName, path) && data.line != undefined) {
                    const newStartLoc = sourceFile.indexToLocation(sourceIndex! + result.locationToIndex({ line: data.line, column: data.column || 0 }));
                    (data as any).line = newStartLoc.line;
                    if (data.column != undefined) {
                        (data as any).column = newStartLoc.column;
                    }
                    if (data.endLine != undefined) {
                        ...
                        (data as any).endLine = newEndLoc.line;
                        if (data.endColumn != undefined) {
                            (data as any).endColumn = newEndLoc.column;
                        }
                    }
                }
    ...
    getModule(file: digo.File) {
        if (file.initialPath) {
            return this.modules[file.initialPath.toLowerCase()]!;
        } else {
            return (file as any)[this.id] as Module;
        }
    }
    ...
    setModule(file: digo.File, module: Module) {
        if (file.initialPath) {
            this.modules[file.initialPath.toLowerCase()] = module;
        } else {
            (file as any)[this.id] = module;
        }
    }
